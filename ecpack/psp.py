...
def append_text(program, stack, text, i, j):
    for line in text[i:j].splitlines(True):
        t_type = 't' if line.endswith("\n") or len(line.strip()) > 0 else 'w'
        program.append((t_type, I(stack) + '__r.append({})'.format(repr(line))))
    return j
...
def psp(text, namespace, separator="%"):
    ...
    Python statments do not have to follow any kind of indentation,
